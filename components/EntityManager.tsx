...
export const EntityManager: React.FC<Props> = ({ entities, ledger, currentFarmId, currentLocationId, onAddEntity, onUpdateEntity, onDeleteEntity, onAddPayment }) => {
    ...
    const handleSave = () => {
        ...
        const newEntity: Entity = {
            ...
            notes: form.notes,
            farmId: form.id ? form.farmId : (currentFarmId || undefined)
        };
        ...
        <div className="space-y-6 animate-fade-in">
            {!currentFarmId && (
                <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">Select a farm above to view and add vendors, customers, and Palai clients. Entity Registry is farm-wise — you only see and add entities for the selected farm.</span>
                </div>
            )}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Entity Registry</h2>
                    <p className="text-sm text-gray-500">Vendors, customers, and Palai clients — scoped to selected farm</p>
                </div>
                <button
                    onClick={() => {
                    ...
                    className={`flex items-center gap-2 text-sm font-bold px-6 py-3 rounded-xl transition-colors shadow-lg ${currentFarmId ? 'text-white bg-emerald-600 hover:bg-emerald-700 shadow-emerald-100' : 'text-gray-400 bg-gray-200 cursor-not-allowed shadow-gray-100'}`}
                >
